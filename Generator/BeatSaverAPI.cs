...
namespace BeatSaverAPI
{
    ...
    public class BeatSaverAPISong
    {
        ...
        public string ErrCode { get; set; } = "200";
        public long TimeNeeded { get; set; } = 0;
        public long TimeNeededGetDownloadSize { get; set; } = 0;
    ...
    public class BeatSaverAPIInteractor
    {
        ...
        {
            BeatSaberSong c = JsonSerializer.Deserialize<BeatSaberSong>(json, new JsonSerializerOptions { IgnoreNullValues = true });
            return c;
        }

        public async void Log(String message, int Type)
        {
            DateTime t = DateTime.Now;
            if (Type == 0)
            {
                Console.Out.WriteLine(t.Hour.ToString("d2") + ":" + t.Minute.ToString("d2") + ":" + t.Second.ToString("d2") + " [Info]      " + message);
            }
            else if (Type == 1)
            {
                Console.Out.WriteLine(t.Hour.ToString("d2") + ":" + t.Minute.ToString("d2") + ":" + t.Second.ToString("d2") + " [Error]     " + message);
            }
        }
        ...
        internal async Task<BeatSaverAPISong> BeatSaverAPISongHash(string Hash)
        {
            ...
            foreach (BeatSaverAPISong song in songs)
            {
                if (song.hash.ToLower() == Hash.ToLower() || song.key.ToLower() == Hash.ToLower())
                {
                    song.GoodRequest = true;
                    return song;
                }
            ...
            while (RateLimit)
            {
                ...
                cl.Headers.Add("user-agent", "Beat Saver API Interactor/1.0");
                try
                {
                    String tmp = cl.DownloadString(BeatSaverAPIBaseLink + "maps/by-hash/" + Hash.ToLower());
                    BeatSaverResult = JsonSerializer.Deserialize<BeatSaverAPISong>(tmp, new JsonSerializerOptions { IgnoreNullValues = true });
                ...
                catch (WebException e)
                {
                    ...
                    {
                        Console.WriteLine("Exceeded Rate limit waiting 5 sec");
                    ...
                    else if (response.Contains("404"))
                    {
                        ...
                        RateLimit = false;
                        BeatSaverResult.ErrCode = response;
                    }
                    else
                    {
                        BeatSaverResult.GoodRequest = false;
            ...
            watch.Stop();
            BeatSaverResult.TimeNeeded = watch.ElapsedMilliseconds;
        ...
        internal async Task<BeatSaverAPISong> BeatSaverAPISongKey(string Key)
        {
            ...
            foreach (BeatSaverAPISong song in songs)
            {
                if (song.key.ToLower() == Key.ToLower() || song.hash.ToLower() == Key.ToLower())
                {
                    song.GoodRequest = true;
                    return song;
                }
        ...
        internal async Task<BeatSaverAPISearchResult> SearchTextAPI(String text)
        {
            ...
            while (RateLimit)
            {
                ...
                try
                {
                    ...
                    BeatSaverResult = JsonSerializer.Deserialize<BeatSaverAPISearchResult>(tmp, new JsonSerializerOptions { IgnoreNullValues = true });
                    RateLimit = false;
